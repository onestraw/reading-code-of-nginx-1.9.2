...
static ngx_http_module_t  ngx_http_mytest_upstream_module_ctx =
{
    ...
    ngx_http_mytest_upstream_create_loc_conf,
                                       /* create location configuration */
    ngx_http_mytest_upstream_merge_loc_conf
...
static void* ngx_http_mytest_upstream_create_loc_conf(ngx_conf_t *cf)
{
    ...
    /*
     * upstream模块要求hide_headers成员必须要初始化（upstream在解析完上游服务器
     * 返回的包头时，会调用ngx_http_upstream_process_headers方法, 按照
     * hide_headers成员将本应转发给下游的一些http头部隐藏），这里将它赋为
     * NGX_CONF_UNSET_PTR ，是为了在merge合并配置项方法中使用upstream模块提供的
     * ngx_http_upstream_hide_headers_hash方法初始化hide_headers 成员
     */
...
/* 
 * 这里定义的mytest_upstream_create_request方法用于创建发送给上游服务器的
 * HTTP请求，upstream模块将会回调它 
 */
static ngx_int_t
mytest_upstream_create_request(ngx_http_request_t *r)
{
    ...
    //必须由内存池中申请内存，这有两点好处：在网络情况不佳的情况下,
    //向上游服务器发送请求时，可能需要epoll多次调度send发送才能完成，
...
mytest_process_status_line(ngx_http_request_t *r)
{
    ...
    ngx_http_mytest_upstream_ctx_t  *ctx;

    //上下文中才会保存多次解析http响应行的状态，首先取出请求的上下文
    ctx = ngx_http_get_module_ctx(r, ngx_http_mytest_upstream_module);
    if (ctx == NULL) {
        return NGX_ERROR;
    }
    ...
    /* 
     * 以下表示解析到完整的http响应行，这时会做一些简单的赋值操作，
     * 将解析出的信息设置到r->upstream->headers_in结构体中，upstream解析完
     * 所有的包头时，就会把headers_in中的成员设置到将要向下游发送的
     * r->headers_out结构体中，也就是说，现在我们向headers_in中设置的信息，
     * 最终都会发往下游客户端。为什么不是直接设置r->headers_out而要这样
     * 多此一举呢？这是因为upstream希望能够按照ngx_http_upstream_conf_t
     * 配置结构体中的hide_headers等成员对发往下游的响应头部做统一处理 
     */
    if (u->state)
    {
